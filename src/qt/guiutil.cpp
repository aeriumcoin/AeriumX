...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a AeriumX address (e.g. %1)").arg("A7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
#endif
...
{
    // return if URI is not valid or is no AeriumX: URI
...
{
    // Convert aeriumx:// to aeriumx:
    //
    //    Cannot handle this later, because aeriumx:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open aeriumx.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "AeriumX.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for AeriumX.lnk
...
{
    return GetAutostartDir() / "aeriumx.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a aeriumx.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=AeriumX\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the aeriumx app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add aeriumx app to startup item list
